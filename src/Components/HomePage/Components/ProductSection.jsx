...
const ProductSection = ({ products, heading, path, addCart, wishList }) => {
  ...
  return (
    <div className="bg-white w-full p-4 mobile:p-4">
      ...
      <h1 className="text-2xl font-bold text-text-blue">{heading}</h1>
      {products.length > 0 ? (
        // <div className="grid grid-cols-1 sm-md:grid-cols-2 lg:grid-cols-3 gap-4 p-4 mobile:p-4">
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-3 gap-4 p-4 mobile:p-4">

          {products.map((item, index) => (
            <div
              key={item.id}
              className="snap-center border rounded-sm bg-white shrink-0 m-3 mobile:m-0 "
            >
              <div className="relative rounded-t-sm bg-slate-100 m-2">
                <img
                  onClick={(e) => {
